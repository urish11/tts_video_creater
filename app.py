...
def generate_text_with_claude(prompt: str, anthropic_api_key: str = anthropic_api_key, model: str = "claude-opus-4-20250514", temperature: float = 1.0, max_retries: int = 3): # claude-3-opus-20240229, claude-3-sonnet-20240229, claude-3-haiku-20240307
...
def generate_audio_with_timestamps(text, client,lang, voice_id="alloy"):
    ...
    response = client.audio.speech.create(
        model="gpt-4o-mini-tts",#gpt-4o-mini-tts  tts-1-hd
        voice=voice_id,
        input=text,
    ...
    two_letter_lang = lang_map[lang.strip().title()]
        ...
        for _, row in df.iterrows():
            ...
            for i in range(count):
                ...
                prompt = f"""Write a JSON-formatted script for a 10–15 second video ad promoting: "{topic}" in language "{lang}".
                                ...
                                • Use **simple**, engaging, curiosity-driven text. Avoid , fake\outlansish promises, or corporate tone. but still be very convincing and eye catching!!! never use "Today" "x% off.." "Get it Now..", find the gray area to make it super enticng but with these limitations!!!!!!!!!!!!!!!!!!
